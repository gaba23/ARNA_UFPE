<?php
session_start();
if (!isset($_SESSION['email'])) {  // redireciona usuários não autenticados à página de login
    header("Location: login.php");
    exit();
}
?>

<!DOCTYPE html>
<html lang="pt-br">
<head>
    <title>Resultado</title>
    <link rel="stylesheet" type="text/css" href="../static/home.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.3/css/all.min.css">
    <link rel="icon" type="image/png" href="../static/favicon.png">
</head>
<body>
    <header>
        <div class="logo">
            <a href="./home.php">
                ARNA
            </a>
        </div>
        <nav>
            <ul>
                <li class="dropdown">
                    <a href="./home.php">
                        ▼ Analisar
                    </a>
                    <div class="dropdown-content">
                        <a href="/home">CPM</a>
                        <a href="/homePERT">PERT</a>
                        <a href="/monteCarlo">Simulação de Montecarlo</a>
                    </div>
                </li>
                <li><a class="active" href="#">Resultado</a></li>
                <li><a href="./help.php">Ajuda</a></li>
                <li><a href="./contact.php">Contato</a></li>
            </ul>
        </nav>
        <div class="profile dropdown">
            <div class="profile-dropdown dropdown">
                <img src="../static/avatar.png" alt="Avatar" onclick="toggleDropdown()">
                <div class="dropdown-content dropdown">
                    <button onclick="window.location.href='logout.php'">Sair</button>
                </div>
            </div>
        </div>
    </header>

<div class="container" style="margin-bottom: 200px;">
    <h1>Resultado da Análise</h1>
    
    <div class="image-box">
        <div class="image-container">
            <div class="image-border">
                <img src="../resultadosPert/atividades_pert.png" alt="Rede de Atividades">
            </div>
        </div>
    </div>
     <div class="image-box">
        <div class="image-container">
            <div class="image-border">
                <img src="../resultadosPert/gantt_pert.png" alt="Gráfico de Gantt">
            </div>
        </div>
    </div> 
    <!-- <div class="image-box">
        <div class="image-container">
            <div class="image-border">
                <img src="{{ url_for('resultadosPert', path='/diagrama_na_seta.png') }}" alt="Diagram na Seta">
            </div>
        </div>
    </div>  -->

    <div class="download-buttons">
        <a href="#" class="download-button" onclick="openReportModal()">
            <i class="fas fa-chart-bar"></i> Relatório
        </a>
        <a href="../resultadosPert/relatorio.pdf" class="download-button" onclick="openReportModal()">
            <i class="fas fa-file-pdf"></i> Baixar Relatório em PDF
        </a>        
        <a href="{{ url_for('download_png') }}" class="download-button">
            <i class="fas fa-file-image"></i> Baixar PNG
        </a>
        <!-- <a href="{{ url_for('download_xls') }}" class="download-button">
            <i class="fas fa-file-excel"></i> Baixar XLS
        </a> -->
        <a href="#" class="download-button" onclick="openReportModalProbabilidade()">
          <i class="fas fa-chart-bar"></i> Calculadora de probabilidade de término do projeto
        </a>

    </div>
</div>

<!-- Modal -->
<div class="modal" id="reportModal">
    <div class="modal-content">
        <span class="close" onclick="closeReportModal()">&times;</span>
        <p>Caminho Crítico: {{ critical_path | join(" → ") }}</p>
        <p>Tempo Total: {{ tempo_total }}</p>
        <h2>Tabela de Arestas</h2>

        <div class="image-box">
            <div class="image-container">
                <div class="image-border">
                    <img src=".../resultadosPert/tabela_arestas.png" alt="Tabela de Arestas">
                </div>
            </div>
        </div>

        <div class="download-buttons">
            <a href="../resultadosPert/relatorio.pdf" class="download-button">
                <i class="fas fa-file-pdf"></i> Baixar PDF
            </a>
        </div>
    </div>
</div>

<script>
    // Função para abrir o modal de Relatório
    function openReportModal() {
        var modal = document.getElementById("reportModal");
        modal.style.display = "block";
    }

    // Função para fechar o modal de Relatório
    function closeReportModal() {
        var modal = document.getElementById("reportModal");
        modal.style.display = "none";
    }

    // Função para abrir o modal de Probabilidade Gaussiana
    function openReportModalProbabilidade() {
        var modal = document.getElementById("ProbabilidadeModal");
        modal.style.display = "block";
    }

    // Função para fechar o modal de Probabilidade Gaussiana
    function closeReportProbabilidade() {
        var modal = document.getElementById("ProbabilidadeModal");
        modal.style.display = "none";
    }
</script>

<script src="https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.4.0/jspdf.umd.min.js"></script>

<div class="modal" id="ProbabilidadeModal">
    <div class="modal-content">
        <form action="{{ url_for('gauss') }}" method="POST">
            <span class="close" onclick="closeReportProbabilidade()">&times;</span>
            <h2>Calculadora de probabilidade de término do projeto</h2>
            <img src="../static/Tabela Normal - ARNA.png" alt="">
            <p>
                Essa calculadora tem foco em calcular a probabilidade de um projeto terminar em até dado tempo t programado P(X<=t), considerando uma curva normal (Distribuição Tabela Z).
            </p>
    
            <label for="saida">Insira o tempo programado:</label>
            <input type="text" name="t_programado" id="t_programado" placeholder="Tempo programado">
            <button class="probabilidade-button" type="button" onclick="calcularProbabilidade()">Calcular</button>
            <p id="resultado"></p>
        </form>
    </div>
</div>

<script>
    
    const Tempo_total = {{ tempo_total }};
    const Desvio_padrao_projeto = {{ desvio_padrao_projeto }};

    function calcularProbabilidade() {
        const t_programado = parseFloat(document.getElementById("t_programado").value);

        
        if (isNaN(t_programado)) {
            document.getElementById("resultado").innerText = "Por favor, insira um número válido.";
            return;
        }

        const z = (t_programado - Tempo_total) / Desvio_padrao_projeto;
        const probabilidade = cdfNormal(z);

        document.getElementById("resultado").innerText = 
            `Probabilidade de término em até ${t_programado} unidades de tempo: ${(probabilidade * 100).toFixed(2)}%`;
    }

    function cdfNormal(z) {
        return 0.5 * (1 + erf(z / Math.sqrt(2)));
    }

    function erf(x) {
        // Aproximação de Abramowitz e Stegun, Handbook of Mathematical Functions
        const sign = (x >= 0) ? 1 : -1;
        x = Math.abs(x);

        const a1 =  0.254829592,
              a2 = -0.284496736,
              a3 =  1.421413741,
              a4 = -1.453152027,
              a5 =  1.061405429,
              p  =  0.3275911;

        const t = 1.0 / (1.0 + p * x);
        const y = 1.0 - (((((a5 * t + a4) * t) + a3) * t + a2) * t + a1) * t * Math.exp(-x * x);

        return sign * y;
    }
</script>

<footer>
    <div class="footer-content">
        <div class="footer-logo">
            <img src="../static/logoPMD.png" alt="Random Logo">
        </div>
        <div class="footer-links">
            <a href="#">Termos</a>
            <a href="#">Privacidade</a>
            <a href="#">Cookies</a>
        </div>
    </div>
</footer>
</body>
</html>